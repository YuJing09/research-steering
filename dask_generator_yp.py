...
def concatenate(camera_names, time_len):
  ...
  for cword, tword in zip(camera_names, logs_names):
    try:
      with h5py.File(tword, "r") as t5:
        ...
        curvature.append(curvature_value[idxs])
  ...
  return c5x, angle, speed,curvature, filters, hdf5_camera
...
def datagen(filter_files, time_len=1, batch_size=256, ignore_goods=False):
  ...
  c5x, angle, speed,curvature, filters, hdf5_camera = concatenate(filter_names, time_len=time_len)
  ...
  while True:
    try:
      ...
      yield (X_batch, angle_batch, curvature_batch)
